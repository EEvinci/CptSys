...
public class StuDao {
	...
	public ResultSet queryStuRwd(Connection conn, BeanStuInfo stuInfo) throws BaseException{
		...
		try {
			if(StringUtil.isEmpty(stuInfo.getStuId()) && StringUtil.isEmpty(stuInfo.getName()) && StringUtil.isEmpty(stuInfo.getProfess()) && StringUtil.isEmpty(stuInfo.getGrade())){
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是'");
			}
				...
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是'"
						+ " and stu_info.StuId="+ stuInfo.getStuId()
	 ...
	 * @return
	 */
	public ResultSet queryRwdStuCpt(Connection conn, BeanStuInfo stuInfo,BeanCptInfo cpt) {
		
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer("select count(team.RwdGrade) RwdNum,stu_info.StuId, stu_info.Name, "
				+ "stu_info.Profess, stu_info.Grade, stu_info.SClass from stu_info, team, team_up, cpt_info");
		
		try {
			if(StringUtil.isEmpty(cpt.getHoldDay()) && StringUtil.isEmpty(stuInfo.getProfess())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "group by stu_info.StuId order by RwdNum desc");
			}
			else if(StringUtil.isNotEmpty(cpt.getHoldDay()) && StringUtil.isEmpty(stuInfo.getProfess())) { 
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and cpt_info.HoldDay='"+ cpt.getHoldDay() +"' group by stu_info.StuId order by RwdNum desc");
			}
			else if(StringUtil.isEmpty(cpt.getHoldDay()) && StringUtil.isNotEmpty(stuInfo.getProfess())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and stu_info.Profess='"+ stuInfo.getProfess() +"' group by stu_info.StuId order by RwdNum desc");
			}
			else if(StringUtil.isNotEmpty(cpt.getHoldDay()) && StringUtil.isNotEmpty(stuInfo.getProfess())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and cpt_info.HoldDay='"+ cpt.getHoldDay() +"' and stu_info.Profess='"+ stuInfo.getProfess() 
						+"' group by stu_info.StuId order by RwdNum desc");
			}
			PreparedStatement pstmt = conn.prepareStatement(sb.toString().replaceFirst("and", "where"));
			rs = pstmt.executeQuery();
	 ...
	 * @return
	 */
	public ResultSet cntRwdStuCpt(Connection conn, BeanStuInfo stuInfo,BeanCptInfo cpt) {
		
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer("select count(team.RwdGrade) RwdNum, team.RwdGrade, stu_info.Name, stu_info.StuId, stu_info.Profess, "
				+ "stu_info.Grade, stu_info.SClass from stu_info, team, team_up, cpt_info ");
		
		try {
			if(StringUtil.isEmpty(stuInfo.getProfess()) && StringUtil.isEmpty(cpt.getHoldDay())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			else if(StringUtil.isNotEmpty(stuInfo.getProfess()) && StringUtil.isEmpty(cpt.getHoldDay())) { 
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and stu_info.Profess='"+ stuInfo.getProfess() +"' group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			else if(StringUtil.isEmpty(stuInfo.getProfess()) && StringUtil.isNotEmpty(cpt.getHoldDay())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and cpt_info.HoldDay='"+ cpt.getHoldDay() +"' group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			else if(StringUtil.isNotEmpty(stuInfo.getProfess()) && StringUtil.isNotEmpty(cpt.getHoldDay())) {
				sb.append(" and stu_info.StuId = team_up.StuId and team_up.TeamId = team.TeamId and team_up.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and stu_info.Profess='"+ stuInfo.getProfess() +"' and cpt_info.HoldDay='"+ cpt.getHoldDay() 
						+"' group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			PreparedStatement pstmt = conn.prepareStatement(sb.toString().replaceFirst("and", "where"));
			rs = pstmt.executeQuery();
