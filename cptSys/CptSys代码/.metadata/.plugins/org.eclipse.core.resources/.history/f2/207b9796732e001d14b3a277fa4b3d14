...
public class AdminCntStuRwdNumInterFrm extends JInternalFrame {
	...
	public AdminCntStuRwdNumInterFrm() {
		...
		this.fillTable(new BeanStuInfo());
	}
	 ...
	 * @param cptInfo
	 */
	private void fillTable(BeanStuInfo stu) {
		...
		try {
			conn = dbUtil.getConn();
			ResultSet rs = stuDao.queryRwdStuCpt(conn,stu);
			while(rs.next())
			{
				Vector v = new Vector();
