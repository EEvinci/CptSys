...
public class TeamDao {
	...
	public int update(Connection conn, BeanTeam team) throws BaseException{
		...
		int rs = 0;
		//time 
		String sql = "update team set TeamName=?, MemNum=?, RwdGrade=?, IsFinCpt=? where TeamId=? ";//and time>? 需要输入竞赛编号cptinfo.getHoldDay
	 ...
	 * 获取队伍获奖信息
	 */
	public ResultSet queryRwdInfo(Connection conn,BeanTeam team)throws BaseException{

		ResultSet rs = null;
		StringBuffer sb = new StringBuffer("select cpt_info.CptId,team.TeamId,team.TeamName,team.RwdGrade,team.IsFinCpt "
				+ "from team,register,cpt_info ");
		
		try {
			if(StringUtil.isNotEmpty(team.getTeamId())){
				sb.append(" and team.TeamId = register.TeamId and register.CptId = cpt_info.CptId and team.IsFinCpt = '是' "
						+ "and register.RegiTime > cpt_info.HoldDay and TeamId = " + team.getTeamId());
			}
			PreparedStatement pstmt = conn.prepareStatement(sb.toString().replaceFirst("and", "where"));
			rs = pstmt.executeQuery();
	 ...
	 * @throws BaseException
	 */
	public int updateRwdInfo(Connection conn, BeanTeam team) throws BaseException{
