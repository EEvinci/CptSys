 ...
 */
	...
	public ResultSet queryCptRegi(Connection conn, BeanCptInfo cptInfo) throws BaseException{
		...
		ResultSet rs = null;
		String sql = "select stu_info.StuId, stu_info.Name, team.TeamId, cpt_info.CptName, cpt_itm_info.CptItemName, "
				+ "stu_info.SClass, stu_info.Grade, stu_info.Profess, register.RegiTime, register.RegiStatus "
				+ "from cpt_info,cpt_itm_info,stu_info,team,team_up,register where team_up.TeamId = team.TeamId "
				+ "and team_up.StuId = stu_info.StuId and team.TeamId = register.TeamId and register.CptId = cpt_info.CptId "
				+ "and cpt_info.CptItemId = cpt_itm_info.CptItemId and cpt_info.CptName like '%?%' and cpt_info.HoldYear = '?'";
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
			
			pstmt.setString(0, cptInfo.getCptName());
			pstmt.setString(1,cptInfo.getHoldYear());
