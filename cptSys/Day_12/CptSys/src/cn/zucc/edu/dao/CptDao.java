 ...
 */
	...
	public ResultSet queryCptRegi(Connection conn, BeanCptInfo cptInfo) throws BaseException{
		...
		try {
			if(StringUtil.isEmpty(cptInfo.getCptName()) && StringUtil.isEmpty(cptInfo.getCptId())){
			...
			else if(StringUtil.isNotEmpty(cptInfo.getCptName()) && StringUtil.isEmpty(cptInfo.getCptId())){
				sb.append(" and team_up.TeamId = team.TeamId and team_up.StuId = stu_info.StuId and team.TeamId = register.TeamId "
						+ "and register.CptId = cpt_info.CptId and cpt_info.CptItemId = cpt_itm_info.CptItemId "
						+ "and cpt_info.CptName like '%"+ cptInfo.getCptName() +"%'");
			}
			else if(StringUtil.isEmpty(cptInfo.getCptName()) && StringUtil.isNotEmpty(cptInfo.getCptId())){
				sb.append(" and team_up.TeamId = team.TeamId and team_up.StuId = stu_info.StuId and team.TeamId = register.TeamId "
						+ "and register.CptId = cpt_info.CptId and cpt_info.CptItemId = cpt_itm_info.CptItemId "
						+ "and cpt_info.HoldYear = "+ cptInfo.getCptId());
			}
			else if(StringUtil.isNotEmpty(cptInfo.getCptName()) && StringUtil.isNotEmpty(cptInfo.getCptId())){
				sb.append(" and team_up.TeamId = team.TeamId and team_up.StuId = stu_info.StuId and team.TeamId = register.TeamId "
						+ "and register.CptId = cpt_info.CptId and cpt_info.CptItemId = cpt_itm_info.CptItemId "
						+ "and cpt_info.CptName like '%"+ cptInfo.getCptName() +"%' and cpt_info.HoldYear = "+ cptInfo.getCptId());
			}
			PreparedStatement pstmt = conn.prepareStatement(sb.toString().replaceFirst("and", "where"));
			rs = pstmt.executeQuery();
			return rs;
	 ...
	 * @return
	 */
	public ResultSet queryRwdStuCpt(Connection conn, BeanStuInfo stuInfo,BeanTeamUp teamup) {
		
		ResultSet rs = null;
		StringBuffer sb = new StringBuffer("select count(team.RwdGrade) RwdNum,team.RwdGrade, stu_info.Name, cpt_info.CptName, "
				+ "cpt_info.CptId, stu_info.StuId,  team.TeamId, team.TeamName, stu_info.Profess, stu_info.Grade, stu_info.SClass "
				+ "from stu_info, team, team_up, cpt_info, register ");
		
		try {
			if(StringUtil.isEmpty(stuInfo.getGrade()) && StringUtil.isEmpty(teamup.getTeamId())) {
			...
			else if(StringUtil.isNotEmpty(stuInfo.getGrade()) && StringUtil.isEmpty(teamup.getTeamId())) { 
				sb.append(" and stu_info.StuId=team_up.StuId and team.TeamId=team_up.TeamId "
						...
						+ "and stu_info.Grade='"+ stuInfo.getGrade() +"' group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			else if(StringUtil.isEmpty(stuInfo.getGrade()) && StringUtil.isNotEmpty(teamup.getTeamId())) {
				sb.append(" and stu_info.StuId=team_up.StuId and team.TeamId=team_up.TeamId "
						+ "and team_up.TeamId = register.TeamId and cpt_info.CptId = register.CptId and team.IsFinCpt = '是' "
						+ "and team_up.TeamId="+ teamup.getTeamId() +" group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			else if(StringUtil.isNotEmpty(stuInfo.getGrade()) && StringUtil.isNotEmpty(teamup.getTeamId())) {
				sb.append(" and stu_info.StuId=team_up.StuId and team.TeamId=team_up.TeamId "
						+ "and team_up.TeamId = register.TeamId and cpt_info.CptId = register.CptId and team.IsFinCpt = '是' "
						+ "and stu_info.Grade='"+ stuInfo.getGrade() +"' and team_up.TeamId="+ teamup.getTeamId() 
						+" group by stu_info.StuId order by RwdNum desc, team.RwdGrade");
			}
			PreparedStatement pstmt = conn.prepareStatement(sb.toString().replaceFirst("and", "where"));
			rs = pstmt.executeQuery();
