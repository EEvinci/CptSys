...
public class TeamUpDao {
	...
	public int add(Connection conn, BeanTeamUp teamup) throws BaseException{
		...
		int n = 0;
		String sql = "insert into team_up(StuId,StuName,TeamId,TeamName,CptId,IsLeader,IsPtcpt) values(?,?,?,?,?,?,?)";
		...
		try {
			...
			pstmt.setString(6,teamup.getIsLeader());
			pstmt.setString(7,teamup.getIsPtcpt());
